...
class RAGSystem:
    ...
    def _add_to_history(self, user_id: str, user_message: str, bot_response: str, classification_type: str = None, classification_program: str = None):
        """Добавляет пару сообщений в историю, включая классификацию вопроса пользователя."""
        if not user_id:
            return
        ...
        self.conversation_history[user_id].append({
            ...
            'classification_program': classification_program
        })
    ...
    def _classify_question(self, question: str, user_id: str = None) -> dict:
        """Классифицирует вопрос и извлекает программу если нужно. Передаёт в prompt 3 последних пар 'вопрос-ответ' и последнюю обсуждаемую программу."""
        try:
            llm = ChatOpenAI(
                model=DEEPSEEK_MODEL,
                api_key=DEEPSEEK_API_KEY,
            ...
            if user_id and user_id in self.conversation_history:
                # Собираем последние 3 пары (user, bot)
                for entry in reversed(self.conversation_history[user_id]):
                    if 'user' in entry and 'bot' in entry:
                        last_pairs.append((entry['user'], entry['bot']))
                    if len(last_pairs) >= 3:
                        break
                for entry in reversed(self.conversation_history[user_id]):
                    if entry.get('classification_type') == 'specific' and entry.get('classification_program'):
                        last_program = entry['classification_program']
                        break
            last_pairs = list(reversed(last_pairs))  # хронологический порядок
            last_pairs_str = "\n".join([f"{i+1}. Пользователь: {q}\n   Бот: {a}" for i, (q, a) in enumerate(last_pairs)])
            last_program_str = f"Последний вопрос был о программе: {last_program}." if last_program else ""
            template = """
            ...
            История последних сообщений (пары вопрос-ответ):
            {last_pairs_str}
            ...
            - type = "recommendation" если:
                * Описывают свой опыт/навыки и просят совет о программах или предметах, и ЯВНО указывают программу (например, "на программе X", "по программе X", "в программе X")
                * Просят рекомендации на основе background ("мой опыт такой-то, что подходит?") и указывают программу
                * Спрашивают какие предметы/курсы выбрать с описанием опыта и указывают программу
                * Просят помочь с выбором на основе их профиля и указывают программу
                * Есть фразы "я работаю", "мой опыт", "посоветуешь" с контекстом опыта и указана программа
            ...
            {{{{"type": "recommendation", "program": null}}}} или {{{{"type": "general", "program": null}}}} или {{{{"type": "specific", "program": "название программы"}}}}
            """
            prompt = ChatPromptTemplate.from_template(template)
            ...
            response = chain.invoke({
                ...
                "last_program_str": last_program_str,
                "last_pairs_str": last_pairs_str
            }).strip()
            import json
            try:
    ...
    def ask(self, question: str, user_id: str = None) -> str:
        """Задает вопрос RAG системе и возвращает ответ. Сохраняет последнюю выбранную программу в истории, переключается если указана другая."""
        ...
        try:
            ...
            # --- LOGIC: track last selected program, switch if user specifies another ---
            last_program = None
            if user_id and user_id in self.conversation_history:
                for entry in reversed(self.conversation_history[user_id]):
                    if entry.get('classification_type') == 'specific' and entry.get('classification_program'):
                        last_program = entry['classification_program']
                        break
            # Если пользователь явно указал новую программу — переключаемся
            if question_type == 'specific':
                if not target_program and last_program:
                    target_program = last_program
                    print(f"DEBUG: Используем последнюю программу из истории: {target_program}")
                    question = f"{question} (на программе {target_program})"
                elif target_program and last_program and target_program != last_program:
                    print(f"DEBUG: Переключение на новую программу: {target_program}")
            ...
            if user_id:
                self._add_to_history(user_id, question, response.strip(), question_type, target_program)
    ...
    def _handle_recommendation_with_program(self, enhanced_question: str, target_program: str, user_id: str, original_question: str) -> str:
        ...
        # Сохраняем в историю (тип всегда recommendation, программа — target_program)
        if user_id:
            self._add_to_history(user_id, f"Программа: {target_program}", response.strip(), 'recommendation', target_program)
        return response.strip()
